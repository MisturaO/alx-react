...
export default class Notifications extends PureComponent{
    ...
    render(){
        const { listNotifications, handleHideDrawer, handleDisplayDrawer, displayDrawer, markNotificationAsRead} = this.props; //Using destructuring assignment to extract the displayDrawer and listNotifications properties from the props object. 
                ...
                <div>
                    ...
                    listNotifications.map(({id, __html, type, value}) => {
                        return <NotificationItem className={css(style.NotificationItemRed, style.smallBorder, style.notiButtonStyle)} 
                          markNotificationAsRead={markNotificationAsRead} key={id} type={type} value={value} html={__html}/>//Passing expected data types(prop-types)values to child component's props to make sure the props get the right data type values they are defined for(e.g. type is expecting a string and it's values should be a string which has been defined by shape in the NotificationItemShape file) through this parent's prop(listNotifications)
                      })) : (
