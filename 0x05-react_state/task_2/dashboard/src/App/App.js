...
class App extends Component {// The GrandParent component, that renders all Parent components
  ...
  render(){
    const { displayDrawer, user } = this.state //destructures the state properties, so as to access them directly
    const {logOut} = this.props

    return (
      <React.Fragment>
        <AppContext.Provider value={{ user, logOut}}>{/* Sets the value to the 'user' and 'logOut' function using local state */}
          <div className={css(appStyle.appClass)} data-testid="display-drawer">
            <Notifications
